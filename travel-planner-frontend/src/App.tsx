...
const App: React.FC = () => {
  ...
  // Helper function to get attraction description
  const getAttractionDescription = (attraction: any): string => {
    // Use description from backend (generated by formatter agent)
  ...
  const renderExecutionSummary = (executionLog: AgentLog[]) => {
    ...
    const agentNames: { [key: string]: string } = {
      ...
      checker_agent: "Checker Agent",
    };

    return (
      <Card
        title={<Title level={4} style={{ margin: 0, fontFamily: "Georgia, serif", letterSpacing: "0.5px" }}>Execution Summary</Title>}
        style={{ marginBottom: 24 }}
      >
        <Space direction="vertical" size="middle" style={{ width: "100%" }}>
          {executionLog.map((log, index) => {
            const isChecker = log.agent === "checker_agent";
            return (
              <div key={index} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                {isChecker && log.check_result && (
                  <Text type="secondary" style={{ color: log.check_result.passed ? "#52c41a" : "#ff4d4f" }}>
                    {log.check_result.passed ? "Validation Passed" : `Validation Failed (${log.check_result.violations.length} issues)`}
                  </Text>
                )}
              </div>
            );
  ...
  // Helper function to render checker validation details
  const renderCheckerDetails = (checkResult: CheckResult) => {
    if (!checkResult) return null;

    const ruleNames: { [key: string]: string } = {
      json_format: "JSON Format Validation",
      budget: "Budget Validation",
      attractions_count: "Attractions Count Validation",
      hotel_distance: "Hotel Distance Validation",
      flight_completeness: "Flight Completeness Validation"
    };
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ marginBottom: 16 }}>
          <Text type="secondary" style={{ fontSize: 13 }}>
            Validation Status:{" "}
            <Text strong style={{ color: checkResult.passed ? "#52c41a" : "#ff4d4f" }}>
              {checkResult.passed ? "Passed" : "Failed"}
            </Text>
          </Text>
        </div>
        
        {checkResult.check_details && checkResult.check_details.length > 0 && (
          <div style={{ marginTop: 12 }}>
            <Text strong style={{ fontSize: 14, marginBottom: 8, display: "block" }}>
              Validation Results:
            </Text>
            <Space direction="vertical" size="small" style={{ width: "100%" }}>
              {checkResult.check_details.map((detail, index) => {
                const ruleDisplay = ruleNames[detail.rule] || detail.rule;
                ...
                return (
                  <div key={index} style={{ 
                    padding: "8px 12px", 
                    borderRadius: 4,
                    backgroundColor: isPassed ? "#f6ffed" : "#fff2e8",
                    border: `1px solid ${isPassed ? "#b7eb8f" : "#ffd591"}`
                  }}>
                    <Text style={{ fontSize: 13 }}>
                      <Text strong style={{ color: isPassed ? "#52c41a" : "#fa8c16" }}>
                        {isPassed ? "✓" : "✗"} {ruleDisplay}:
                      </Text>{" "}
        ...
        {!checkResult.passed && checkResult.violations && checkResult.violations.length > 0 && (
          <div style={{ marginTop: 16, padding: "12px", backgroundColor: "#fff1f0", borderRadius: 4, border: "1px solid #ffccc7" }}>
            <Text strong style={{ fontSize: 14, color: "#ff4d4f", display: "block", marginBottom: 8 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 20 }}>
              {checkResult.violations.map((violation, index) => (
                <li key={index} style={{ marginBottom: 4 }}>
                  <Text style={{ fontSize: 13 }}>
                    <Text strong>[{violation.rule}]</Text> {violation.message}
                  </Text>
                </li>
              ))}
            </ul>
  ...
  const renderExecutionDetails = (executionLog: AgentLog[]) => {
    ...
    return (
      <Card
        title={<Title level={4} style={{ margin: 0, fontFamily: "Georgia, serif", letterSpacing: "0.5px" }}>Execution Process Details</Title>}
        style={{ marginBottom: 24 }}
      >
        {executionLog.map((log, agentIndex) => {
          ...
          return (
            <div key={agentIndex} style={{ marginBottom: 32 }}>
              <Title level={5} style={{ marginBottom: 16, fontFamily: "Georgia, serif" }}>
              ...
              {!isChecker ? (
                ...
                renderCheckerDetails(log.check_result)
              )}
            </div>
          );
        })}
  ...
  const renderIterations = (result: PlanResult) => {
    if (!result.iteration_logs || result.iteration_logs.length === 0) {
      // Fallback to old format if iteration_logs not available
      return (
        <>
    ...
    return (
      <>
        {result.iteration_logs.map((iterLog, iterIndex) => (
          <div key={iterIndex} style={{ marginBottom: 32 }}>
            <Card
              style={{ 
                marginBottom: 24,
            ...
            >
              ...
              {renderExecutionSummary(iterLog.execution_log)}
              
              {/* Execution Details for this iteration */}
              {renderExecutionDetails(iterLog.execution_log)}
            </Card>
          </div>
        ))}
      </>
  ...
  // Helper function to render checker validation results (deprecated, kept for compatibility)
  const renderCheckerValidation = (result: PlanResult) => {
    if (!result.check_result) return null;

    const checkResult = result.check_result;
    ...
    return (
      <Card
        title={
          <Title level={4} style={{ margin: 0, fontFamily: "Georgia, serif", letterSpacing: "1px" }}>
            Validation Results
          </Title>
        }
      ...
      >
        ...
        {iterations > 0 && (
          <div style={{ marginBottom: 20 }}>
            <Text strong style={{ fontSize: 16, fontFamily: "Arial, sans-serif" }}>
        ...
        <div style={{ 
          padding: "16px", 
          borderRadius: 6,
          backgroundColor: checkResult.passed ? "#f6ffed" : "#fff1f0",
          border: `2px solid ${checkResult.passed ? "#52c41a" : "#ff4d4f"}`
        }}>
          <div style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
            <Text 
              ...
              style={{ 
                fontSize: 18,
                color: checkResult.passed ? "#52c41a" : "#ff4d4f",
                fontFamily: "Georgia, serif",
                letterSpacing: "0.5px"
              }}
            >
              {checkResult.passed ? "✓ Validation Passed" : "✗ Validation Failed"}
            </Text>
          </div>
          ...
          {!checkResult.passed && checkResult.violations && checkResult.violations.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <Text strong style={{ fontSize: 14, color: "#ff4d4f" }}>
                Found {checkResult.violations.length} violation(s):
              </Text>
        ...
        {result.all_check_results && result.all_check_results.length > 1 && (
          <div style={{ marginTop: 24 }}>
            <Title level={5} style={{ marginBottom: 16, fontFamily: "Georgia, serif" }}>
              Iteration History
            </Title>
            <Space direction="vertical" size="small" style={{ width: "100%" }}>
              {result.all_check_results.map((cr, idx) => {
                const status = cr.passed ? "Passed" : `Failed (${cr.violations.length} issues)`;
                const statusColor = cr.passed ? "#52c41a" : "#ff4d4f";
                ...
                return (
                  <div key={idx} style={{ 
                    ...
                    <Text style={{ fontSize: 14 }}>
                      <Text strong>Iteration {idx + 1}:</Text>{" "}
            ...
            {result.trip_config && (
              <Card>
                ...
                <Space direction="vertical" size="middle" style={{ width: "100%" }}>
                  ...
                  {result.hotels && result.hotels.recommended_hotels && result.hotels.recommended_hotels.length > 0 && (
                    <div>
                      <Text strong>Estimated Hotel Cost: </Text>
                      <Text style={{ color: "#52c41a", fontSize: 16 }}>
                        ${(
                          result.hotels.recommended_hotels.reduce((sum: number, hotel: any) => sum + (hotel.total_price || 0), 0) /
                          result.hotels.recommended_hotels.length
                        ).toFixed(2)}
                      </Text>
                    </div>
                  )}
                  {result.flights && result.flights.outbound && result.flights.return && result.hotels && result.hotels.recommended_hotels && result.hotels.recommended_hotels.length > 0 && (
                    <div>
                      <Text strong>Estimated Other Expenses: </Text>
                      <Text style={{ color: "#fa8c16", fontSize: 16 }}>
                        ${(
                          (((result.flights.outbound.recommended_flights?.[0]?.price || 0) + 
                            (result.flights.return.recommended_flights?.[0]?.price || 0)) +
                           (result.hotels.recommended_hotels.reduce((sum: number, hotel: any) => sum + (hotel.total_price || 0), 0) /
                            result.hotels.recommended_hotels.length)) * 0.5
                        ).toFixed(2)}
                      </Text>
                      <Text type="secondary" style={{ marginLeft: 8, fontSize: 12 }}>
                        (50% of flights + hotels)
                      </Text>
                    </div>
                  )}
                </Space>
    ...
    <Layout style={{ minHeight: "100vh", background: "#f5f5f5" }}>
      ...
      {loading ? (
        ...
        <div style={{ maxWidth: 1200, margin: "30px auto" }}>
          {/* Iterations (includes Execution Summary, Details for each iteration) */}
          {renderIterations(result)}
